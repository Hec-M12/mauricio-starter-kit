...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Session rate limits (claude.ai subscription usage)
    let limits = '';
    ...
    if (rateLimits) {
      ...
      if (fiveH != null) {
        ...
        // Use resets_at timestamp (Unix seconds) for accurate rolling window time.
        let remainMins;
        if (fiveH.resets_at) {
          remainMins = Math.max(0, Math.round((fiveH.resets_at * 1000 - Date.now()) / 60000));
        } else {
          remainMins = Math.round((1 - fiveH.used_percentage / 100) * 300);
        }
        const rh = Math.floor(remainMins / 60);
        const rm = remainMins % 60;
        const remainStr = rh > 0 ? `${rh}h${rm > 0 ? rm + 'm' : ''}` : `${rm}m`;
        let color;
        if (pct < 50) color = '\x1b[32m';
        else if (pct < 75) color = '\x1b[33m';
        else color = '\x1b[31m';
        parts.push(`${color}5h:${pct}% ${remainStr}\x1b[0m`);
      }
      ...
      if (sevenD != null) {
        ...
        parts.push(`${color}7d:${pct}%\x1b[0m`);
      }
